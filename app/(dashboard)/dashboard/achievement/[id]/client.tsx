...
export function AchievementContentClient({
  ...
  const generateContent = async (format: Format) => {
    ...
    try {
      const generated = await generateAchievementContent(achievementId, format);
  ...
  const postToSocial = async (platform: "twitter" | "linkedin") => {
    ...
    try {
      await postToSocialClient(content[activeTab], platform);
  ...
  const formatConfig: Record<Format,{ label: string; icon: typeof FileText; description: string }
  > = {
    resume_bullet: {
      label: "Resume",
      icon: FileText,
  ...
  return (
    <Card>
      ...
      <CardContent>
        <Tabs
          value={activeTab}
          onValueChange={(v) => setActiveTab(v as Format)}
        >
          <TabsList className="grid w-full grid-cols-3 mb-6">
            {(
              Object.entries(formatConfig) as [
                Format,
                typeof formatConfig.resume_bullet,
              ][]
            ).map(([format, config]) => (
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute bottom-3 right-3">
                      <Badge variant="secondary" className="text-xs">
                        {content[format].split(/\s+/).filter(Boolean).length}{" "}
                        words
                      </Badge>
                    </div>
                  </div>
