...
export const {
    ...
    events: {
        /**
         * Create user event - runs when a new user is created via the adapter
         * Initializes related records for the new user
         */
        async createUser({ user }) {
            ...
            try {
                ...
                console.log(`[Auth] Created related records for new user: ${user.id}`);
            } catch (error) {
                console.error(`[Auth] Failed to create related records for user ${user.id}:`, error);
            }
        },
