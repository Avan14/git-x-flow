    ...
    callbacks: {
        ...
        async jwt({ token, user, account, profile }) {
            ...
            if (user) {
                ...
                token.role = "user"; // Default role, extend as needed
                ...
                if (profile && account?.provider === "github") {
                    ...
                    if (token.username && user.id) {
                        await prisma.user.update({
                            ...
                            // Ignore update errors (e.g., if username already taken)
                        });
                    }
                }
            }
        ...
        // Authorized callback is now handled in auth.config.ts for edge compatibility
    },
